...
export fn main() callconv(.spirv_fragment) void {
    ...
    const specular_value = pow(max_direction, 32);
    const specular = vector3.splat(0.5) * vector3.splat(specular_value) * light.color;

    const sampled = sampler2d(2, 0, tex_coord_in);

    color_out = sampled * vector4.fromVector3((ambient + diffuse + specular), 1.0);
}

fn reflect(vector: @Vector(3, f32), normal: @Vector(3, f32)) @Vector(3, f32) {
    return vector - vector3.splat(2.0) * vector3.splat(vector3.dot(vector, normal)) * normal;
}
